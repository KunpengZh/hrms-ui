...
class DanweiJitiYiliaobaoxian extends Component {
    constructor(props) {
        super(props);
        this.state = {
            rows: [],
    ...
    handleQuery() {
        ...
        AppStore.queryYiliaobaoxian(query).then(res => {
            if (res.status === 200) {
                this.setState({
                    rows: res.data,
                    fullscreen: false,
                    downloadLink: AppStore.getPreHostURLLink() + '/danweijiti/downloadyiliaobaoxian?criteria=' + JSON.stringify(query)
                })
            } else {
                this.setState({ fullscreen: false });
                AppStore.showError(res.message);
...
export default DanweiJitiYiliaobaoxian;