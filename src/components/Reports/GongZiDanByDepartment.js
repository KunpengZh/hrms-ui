...
class EmpInfoConfig extends Component {
    constructor(props) {
        ...
        this.state = {
            ...
            columns: [
                ...
                { key: 'jibengongzi', name: '工资', width: 100 },
                { key: 'totalJiangjin', name: '奖金', width: 100 },
                { key: 'totalOT', name: '加班费', width: 100 },
                { key: 'tongxunButie', name: '通讯补贴', width: 100 },
                { key: 'nianjin', name: '扣年金', width: 100 },
                { key: 'yanglaobaoxian', name: '扣养老保险', width: 100 },
                { key: 'shiyebaoxian', name: '扣失业保险', width: 100 },
                { key: 'zhufanggongjijin', name: '扣住房公积金', width: 100 },
                { key: 'yiliaobaoxian', name: '扣医疗保险', width: 100 },
                { key: 'totalKouchu', name: '扣除工资', width: 100 },
                { key: 'tax', name: '扣个人所得税', width: 100 },
                { key: 'yicixingjiangjin', name: '年终奖金', width: 100 },
                { key: 'yicixingjiangjinTax', name: '扣年终奖金税', width: 100 },
                { key: 'buchongyiliaobaoxian', name: '扣补充医疗保险', width: 120 },
                { key: 'netIncome', name: '实发工资', width: 100 },
            ],
            activeKey: '',
    ...
    handleQuery() {
        ...
        AppStore.GongZiDanByDepartment(query).then(res => {
            if (res.status === 200) {
                this.setState({
                    rows: res.data,
                    fullscreen: false,
                    downloadLink: AppStore.getPreHostURLLink() + '/gongzidan/downloadbydepartment?criteria=' + JSON.stringify(query)
                })
            } else {
                this.setState({ fullscreen: false });
                AppStore.showError(res.message);
